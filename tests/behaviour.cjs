...
test('Both native JSUI panels paint finite controls; pointer actions dispatch and release',()=>{
 ...
 c.render();u.state(JSON.stringify(c.snapshot()));u.paint();assert.equal(u.hit.filter(h=>h.args[0]==='scene').length,7);assert.equal(u.hit.filter(h=>h.cmd==='pad').length,64);
