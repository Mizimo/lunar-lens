...
「重力」PHRASE 以及其餘五構圖恢復 1.3 的原始形體、運動、寬度與殘影，不再被後加的左／中／右投影改寫。三域改用左青、中琥珀、右桃紅，每側十個不同音域格，直接呈現嚴格聲場測量。低頻仍是完整基座，保留事件、轉場、獨立調參和原低頻判定。

Max 9 / Novation Launchpad Pro MK3。播放任意錄音或接即時輸入；原速 1×，沒有 swing、量化、歌名判斷或逐曲提示點。分析、燈光與觸控效果預設全部使用 Max 原生功能。

![同一音訊幀的中央構圖回滾對照](media/visual-rollback.png)

[1.4.0 的原生聲場校準圖](media/stereo-distribution.png)
...
| **聲場／三域** | 低頻是一個完整基座；中高頻以左青／中琥珀／右桃紅，各十個音域格呈現；低頻完整 |
...
**三域細節可見度** 控制十個音域格的顯示壓縮。三側共用中高頻參考，低頻不壓暗中高頻；相鄰兩格各自代表不同音域，顏色固定標示左右位置。下方三行維持完整低頻基座。[格子音域表](docs/立體聲頻譜.md)

**原有六構圖保持原本視覺邏輯**，不使用三域的形體投影。移除原本控制此投影的「聲場位置」滑桿；形體寬度仍可在五行為中獨立調整。三域格子是固定音域位置，增益、延續與獨看仍可調整。

測量圖的「聚中」要求左右電平接近且高度同相一致，**兩邊都有聲音並不足以聚中**。它是聲像線索，不是中央聲部或人聲分離；同一訊號複製到兩邊和單一居中音源也無法由立體聲錄音區分。PHRASE 偏重變動、BED 偏重延續。[算法、單位和限制](docs/立體聲頻譜.md)
...
專案倉庫：[Mizimo/lunar-lens](https://github.com/Mizimo/lunar-lens)。`v1.2.1` 是低頻回滾基線；`v1.3.0` 保留事件與轉場版；`v1.4.0` 保留嚴格聲場測量版；`v1.4.1` 修正中央視覺回滾，不覆寫舊版。
