...
for(const row of rows){const dt=Math.max(.001,Math.min(.1,row.time-previous));previous=row.time;
 const grids=engines.map((e,i)=>e.step(presentations[i].step(row.features,row.perception,dt,row.params.eventAmount),dt,{...row.params,scene:i<6?i:0,palette:0,focus:i<6?0:i-5,freeze:false,black:false}));
...
svg+=text(32,48,'LUNAR LENS  1.3.0',28,'#edf4fc')+text(32,78,'同一音訊，同一時刻。六種空間構成，各自保留五個聲音行為。',18);
