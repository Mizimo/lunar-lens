// Same captured native features, replayed through all seven production compositions.
import fs from 'node:fs';import path from 'node:path';import {fileURLToPath} from 'node:url';import {createRequire} from 'node:module';
const root=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'..'),require=createRequire(import.meta.url),V=require('../code/lens_visual.js'),P=require('../code/lens_presentation.js');
const source=process.argv[2]||'tmp/role-fixture-native.jsonl',caption=process.argv[3]||'原創混合驗收訊號';
const rows=fs.readFileSync(path.join(root,source),'utf8').trim().split('\n').map(JSON.parse),engines=Array.from({length:12},()=>new V.Engine()),out=[];
const presentations=engines.map(()=>new P.Engine());
...
for(const row of rows){const dt=Math.max(.001,Math.min(.1,row.time-previous));previous=row.time;
 const grids=engines.map((e,i)=>e.step(presentations[i].step(row.features,row.perception,dt,row.params.eventAmount,row.measurements,row.params),dt,{...row.params,scene:i<7?i:0,palette:0,focus:i<7?0:i-6,freeze:false,black:false}));
...
let svg='<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="1430"><rect width="1200" height="1430" fill="#080e18"/><g font-family="Arial,Hiragino Sans GB,sans-serif">';
svg+=text(32,48,'LUNAR LENS  1.4.0',28,'#edf4fc')+text(32,78,'同一音訊，同一時刻。七種空間構成，各自保留五個聲音行為。',18);
V.names.forEach((t,i)=>{const x=32+(i%3)*396,y=122+Math.floor(i/3)*330;svg+=text(x,y,`0${i+1}  ${t}`,19,'#d6e4f2')+text(x+206,y,V.paletteNames[i%6],13)+grid(chosen.grids[i],x,y+22,280);});
svg+=text(32,1140,'同一幀，逐層獨看',21,'#edf4fc');
['衝擊 IMPACT','地基 BODY','前景 PHRASE','鋪陳 BED','細節 DETAIL'].forEach((t,i)=>{const x=32+i*235;svg+=text(x,1177,t,16)+grid(chosen.grids[7+i],x,1198,184);});
svg+=text(32,1410,`${caption} · ${chosen.position.toFixed(2)} s · Native 特徵／正式渲染器重播 · 螢幕 gamma 預覽`,13)+'</g></svg>';
