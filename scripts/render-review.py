"""Render six geometrically different views of the same captured input. Pillow + ffmpeg.
...
text(d,(32,785),'同一幀，逐層獨看',23,'#edf4fc')
for i,t in enumerate(['衝擊 IMPACT','地基 BODY','前景 PHRASE','鋪陳 BED','細節 DETAIL']):
 x=32+i*235;text(d,(x,835),t,17);grid(d,selected['grids'][6+i],x,868,184)
text(d,(32,1070),selected['caption']+f" · {selected['position']:.2f} s · Native 特徵／正式渲染器重播 · 螢幕 gamma 預覽",14)
...
proc=subprocess.Popen(['ffmpeg','-y','-v','error','-f','rawvideo','-pix_fmt','rgb24','-s','1200x840','-r',str(fps),'-i','-','-an','-c:v','libx264','-crf','18','-pix_fmt','yuv420p',str(silent)],stdin=subprocess.PIPE)
for n in range(fps*duration):
 ...
 text(d,(32,800),'原創驗收訊號 · 相同 Native 分析幀 · 正式渲染器重播 · 非實機 LED 拍攝',15)
...
print('Wrote preview.png and 17-second six-composition movie.')
